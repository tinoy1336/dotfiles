 ...
 * FALLBACK (2): when the promptd invocation produces NO reply (service down /
 * unreachable / killed / broken output), the TUI confirm dialog + the yad
 * askpass bridge run instead. A reply whose first line starts with `error:` is
 * an ANSWER, not a dead channel: the router maps that prefix to a non-zero
 * exit, so `error: cancelled` — the user dismissing the window — arrives
 * exactly like a transport failure did and IS the denial, and any other
...
import { argText, clip, safeToolHeader, type HeaderPart } from "./lib/tool-header.ts";
...
function promptdInstances(): string[] {
  ...
  try {
    const map = readFileSync(
      join(checkoutRoot(), "common/shell/route-map.conf"),
      "utf8",
    );
...
export function promptdRequest(
  cmd: string,
  ...
  if (missing) {
    audit({ decision: "router-missing", path: AGS_ROUTE, hint: "SUDO_APPROVE_ROUTE|TINSHELL_HOME" });
    resolve(missing);
    return;
  }
    ...
    const child = execFile(
      ...
      (err, stdout, stderr) => {
        if (settled) return;
        // The reply first: a reply is the ANSWER whatever the exit status,
        // because the router exits non-zero for an `error:` first line. Only
        // the ABSENCE of a reply (spawn failure, kill, empty stdout) is a
        // transport failure.
...
async function runBatch(
  ...
  signal: AbortSignal | undefined,
  onUpdate:
    | ((update: { content: { type: "text"; text: string }[] }) => void)
    | undefined,
  ...
  for (let i = 0; i < commands.length; i++) {
    ...
    if (
      askpass === null &&
      code !== 0 &&
      /a password is required/i.test(output)
    ) {
      onUpdate?.({
        content: [
          {
  ...
  pi.registerTool({
    ...
    renderCall(args, theme) {
      return safeToolHeader(theme, "sudo_approve", () => {
        ...
        const why = argText(args, "justification") ?? argText(first, "justification") ?? argText(first, "command");
        const parts: HeaderPart[] = [["accent", ` ${list.length} command${list.length === 1 ? "" : "s"}`]];
    ...
    async execute(_toolCallId, params, signal, onUpdate, ctx) {
          ...
          content: [
            {
              type: "text",
              text:
                "approval channel died (host crash/restart) — no approval granted; re-issue sudo_approve to open a fresh window",
            },
          ],
      ...
      if (reply.startsWith("error:")) {
        // A reply, not a dead channel: the promptd path answered with its own
        // failure (the router maps an `error:` first line to a non-zero exit,
        // so it arrives exactly like a transport failure did). Record that
        // failure under the call-failed decision — its `reply` field carries
        // the path's own answer, where a transport failure's row carries the
        // child's stderr — then let the documented fallback decide.
        audit({
          decision: "promptd-call-failed",
      ...
      // The channel that is missing is named AHEAD of the request it is about
      // to run through the fallback: a terminal confirm that says nothing about
      // the missing window reads as the intended path.
      const windowMissing =
        reply === ""
      ...
      if (!approved) {
        const reason =
          (await ctx.ui.input(
            "Approval denied - reason (optional)",
            "Press Enter to skip",
          )) ?? "";
