/**
 * lib/tariff — the house price table for the models this machine is billed for.
 *
 * ONE owner, read by both consumers: `deepseek-cost.ts` prices the footer from
 * it, and `fleet/retire.ts` derives the cache-read ratio from it. A second copy
 * of these numbers anywhere would let one consumer decide against last month's
 * table while the other reported this month's.
 *
 * NO RATE IS IN THIS FILE, and none is compiled into it. The table is DATA, read
 * from `tariff.json` in the extensions directory (`PI_TARIFF_CONFIG` overrides the
 * path). It is THIS MACHINE'S PRIVATE TABLE: the extensions that read it are meant
 * to be publishable, so that file must be excluded from any published copy — with
 * `PI_TARIFF_CONFIG` it can live anywhere the pack does not reach, e.g. the agent
 * directory beside `models.json`. The file is read once at module scope, so an
 * edit is live in the NEXT session, never in the running one.
 *
 ...
 * surfaces: `deepseek-cost.ts` registers no pricing at all and leaves the footer
 * empty, while `fleet/retire.ts` returns its own typed refusal naming the same
 * file — the shape it already used for a model whose registry row carries no
 * ratio.
 *
 ...
 *
 * Peak and valley are the two windows of the vendor's own schedule; the clock
 * that selects between them lives in `deepseek-cost.ts`, the only consumer that
 * needs one. The RATIOS below are DERIVED from the configured table, and are
 * identical in both windows and both currency columns for the table in force, so a
 * retirement decision does not depend on the time of day.
 */
...
/**
 * Where the table is read from: `PI_TARIFF_CONFIG` when set, else `tariff.json` at
 * the top of the extensions directory — beside the consumers that read it, and
 * relocatable to machine-private space with that one variable when the extensions
 * are published.
 */
export const TARIFF_CONFIG_PATH: string = (() => {
	const override = process.env.PI_TARIFF_CONFIG?.trim()
	return override && override !== "" ? override : fileURLToPath(new URL("../tariff.json", import.meta.url))
})()
...
function rowOf(value: unknown, where: string): TariffRow | string {
	...
	const unknown = Object.keys(value).find((k) => k !== "cacheHit" && k !== "cacheMiss" && k !== "output")
...
export const TARIFF: TariffLoad = (() => {
	...
	return typeof parsed === "string"
		? { ok: false, path: TARIFF_CONFIG_PATH, reason: `the tariff table is unusable: ${parsed} — ${SHAPE_HINT}` }
...
// One line, once per process, naming the file to write: a session must not have
// to infer from an empty footer that no table was configured.
 ...
 */
export function ratios(table: TariffConfig, column: "usd" | "cny" = "usd", window: Window = "valley"): Ratios {
