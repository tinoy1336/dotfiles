 ...
 * session silently.
 *
 * TWO states only (user 2026-09-11): `on` gates desktop intrusion, `off` does
 * not. The earlier quiet/locked split enforced the same deny-list — the only
 * difference was the wording of the instruction to the model — so it was
 * collapsed. Files written before that still read correctly: quiet and locked
 * both meant "gated", full meant "not gated".
 *
...
export function readFocusState(): FocusState {
	try {
		...
		const raw = JSON.parse(readFileSync(FOCUS_STATE_PATH, "utf8")) as { mode?: string; since?: string };
