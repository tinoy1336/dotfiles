 ...
 *
 * Drives the REAL installed extensions — never a copy:
 ...
 *
 * The fixture block is DERIVED from the live canon store at run time through the
...
const TOOLS = [
	...
	{ type: "function", function: { name: "fleet", description: "crew", parameters: { type: "object" } } },
];
...
const shapes: Array<[string, Record<string, unknown>, string]> = [
	...
	["anthropic system block", { system: [{ type: "text", text: base, cache_control: { type: "ephemeral" } }] }, "block"],
];
...
await emit("before_provider_request", {
	payload: { model: "deepseek-flash", messages: [{ role: "system", content: typedText }], tools: TOOLS },
});
await tick();
...
const wakePayload = { model: "deepseek-flash", messages: [{ role: "system", content: base }], tools: TOOLS };
...
check("GUARD: unfixed wake turn writes a row", afterUnfixed.length === 2, `rows=${afterUnfixed.length}`);
...
check("GUARD: row says the system prompt moved", Array.isArray(row?.changed) && (row?.changed as string[]).includes("sys"));
check("GUARD: tools untouched", (row?.changed as string[]).includes("tools") === false);
check(
