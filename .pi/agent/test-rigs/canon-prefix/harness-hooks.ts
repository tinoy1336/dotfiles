 ...
 * helpers) against the REAL installed canon package, imported at run time.
 ...
 * before_agent_start hook over the LIVE store — not a recorded fixture — so the
 * bytes under test are always the ones the extension renders today.
 *
...
const emit = (ev: string, event: unknown) =>
...
const typedPayload = { model: "deepseek-flash", messages: [{ role: "system", content: typedText }], tools: [{ type: "function", function: { name: "read" } }] };
await emit("before_provider_request", { payload: typedPayload });
check("typed request untouched (byte-identical)", (typedPayload.messages[0] as { content: string }).content === typedText);
...
const wakePayload = { model: "deepseek-flash", messages: [{ role: "system", content: base }], tools: [{ type: "function", function: { name: "read" } }] };
await emit("before_provider_request", { payload: wakePayload });
check("wake request normalized in place", (wakePayload.messages[0] as { content: string }).content === typedText);
check("wake request logged once as prompt-normalized", logs().length === 1 && logs()[0]?.kind === "prompt-normalized", JSON.stringify(logs()[0]?.kind));
check("wake log carries sizes", typeof logs()[0]?.detail.baseChars === "number" && typeof logs()[0]?.detail.canonicalChars === "number");

// 3. second request of the same wake run: repaired again, but not re-logged.
const wake2 = { model: "deepseek-flash", messages: [{ role: "system", content: base }], tools: [{ type: "function", function: { name: "read" } }] };
await emit("before_provider_request", { payload: wake2 });
check("second wake request normalized", (wake2.messages[0] as { content: string }).content === typedText);
...
const anomaly = { model: "deepseek-flash", messages: [{ role: "system", content: base }], tools: [{ type: "function", function: { name: "read" } }] };
...
check("foreign shape untouched", JSON.stringify(foreign).includes("BASE-SYSTEM-PROMPT") && !JSON.stringify(foreign).includes("## Canon"));
check(
...
console.log(`\nfixture: live block ${block.length} chars (from canon before_agent_start)`);
finish("HOOK", count());
