 ...
 * ONE fake pi chain, over the REAL installed modules, with the fixture block
...
const payload = (content: string) => ({ model: "deepseek-flash", messages: [{ role: "system", content }], tools: TOOLS });
...
for (const order of [["canon", "logger"], ["logger", "canon"]] as const) {
	...
	check(`[${label}] baseline row written`, baseline.length === 1, JSON.stringify(baseline[0]?.changed));
	...
	check(`[${label}] logger fingerprint == the prompt canon produced`, baseline[0]?.sysChars === typedText.length, `sysChars=${baseline[0]?.sysChars} vs ${typedText.length}`);
...
console.log(`\nfixture: live block ${block.length} chars; composed chains run in both orders.`);
finish("CHAIN", count());
