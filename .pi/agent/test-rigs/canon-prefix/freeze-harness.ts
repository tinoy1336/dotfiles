 ...
 *
 * Drives the REAL installed child-prompt-freeze.ts (imported at run time), whose
 * own `systemPromptSlot` import pulls the shared seam package — no spliced copy of
 * the slot code. cache-prefix-log.ts is the real logger. No provider call, no
 * session, no child process.
 *
...
const logs = (): Array<{ source: string; kind: string; detail: Record<string, unknown> }> =>
	((globalThis as unknown as { __hookLog?: Array<{ source: string; kind: string; detail: Record<string, unknown> }> }).__hookLog ?? []).filter(
		(l) => l.source === "child-prompt",
	);
const rows = (): Array<Record<string, unknown>> =>
	readFileSync(LOG, "utf8").split("\n").filter(Boolean).map((l) => JSON.parse(l));
const systemMsg = (content: string) => ({ model: "deepseek-flash", messages: [{ role: "system", content }], tools: [{ type: "function", function: { name: "read" } }] });
const contentOf = (p: Record<string, unknown>) => (p.messages as Array<{ content: string }>)[0].content;
...
check("child session: hooks registered", (handlers.before_agent_start?.length ?? 0) === 1 && (handlers.before_provider_request?.length ?? 0) === 1);
...
check("woken run restored byte-exactly", contentOf(p2) === REWRITTEN, `len ${contentOf(p2).length} vs ${REWRITTEN.length}`);
check("woken run differs from what arrived", contentOf(p2) !== BASE);
check("restore logged once as prompt-normalized", logs().length === 1 && logs()[0]?.kind === "prompt-normalized", JSON.stringify(logs().map((l) => l.kind)));
check("log carries both sizes", logs()[0]?.detail.arrivedChars === BASE.length && logs()[0]?.detail.canonicalChars === REWRITTEN.length);
...
check("change logged as canonical-refresh", logs().some((l) => l.kind === "canonical-refresh"), JSON.stringify(logs().map((l) => l.kind)));
...
check("developer-role shape restored", (p7.messages[0] as { content: string }).content === REWRITTEN_2);
...
check("foreign shape left untouched", JSON.stringify(p9).includes(AGENT_PROMPT.split("\n")[0]) && !JSON.stringify(p9).includes(BOUNDARY));
...
// B7. wake-first process: nothing pinned, nothing invented.
{
	...
	check("wake-first process says so once", logs().length === before + 1 && logs()[logs().length - 1]?.kind === "canonical-unknown");
}
...
// ── C. child-side visibility (the real logger, in the same chain) ───────────
{
	...
	await tick();
	const baseline = rows();
	check("child baseline row written", baseline.length === 1 && baseline[0]?.origin === "prompt", JSON.stringify(baseline[0]?.origin));
	...
	check("GUARD: row names the injected run-start path", row?.origin === "injected", String(row?.origin));
	check(
